    ...
    if (autoRotate && viewMode === "orbital") {
      const animate = (time: number) => {
        if (lastTime !== null) {
          ...
          setRotationAngle((prev) => (prev + delta * 0.018) % 360);
        }
        lastTime = time;
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              <div
                ...
                style={{ ...nodeStyle, willChange: "transform, opacity", transition: isExpanded ? "all 0.5s cubic-bezier(0.4,0,0.2,1)" : "opacity 0.3s ease" }}
                onClick={(e) => {
                  e.stopPropagation();
                  toggleItem(item.id);
                }}
